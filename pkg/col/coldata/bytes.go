...
// eight bytes of each []byte. It is used for byte comparison fast paths. It
// may return nil if the cardinality of abbreviated values is very low. In this
// case, it will likely be slower to use the byte comparison fast path because
// the abbreviated values will frequently be equal, incurring extra computation
// that won't short-circuit the full value comparison very often.
//
...
func (b *Bytes) Abbreviated() []uint64 {
	...
	// Initialize a hyperloglog sketch with precision 10, which allocates 1 KiB
	// and has an accuracy within 3.3%.
	// TODO: Don't estimate cardinality for less than ~1000 values.
	// const minValsPerDistinctAbbr = 1000
	// i := 0
	// if n > minValsPerDistinctAbbr {
	// 	const precision = 10
	// 	const sparse = false
	// 	hll, err := hyperloglog.NewSketch(precision, sparse)
	// 	if err != nil {
	...
	// 		r[i] = abbreviate(bs)
	//
	// 		// Add up to the first 8 bytes to the HLL sketch.
	// 		if len(bs) > 8 {
	// 			bs = bs[:8]
	// 		}
	// 		hll.Insert(bs)
	//
	// 		// Check the cardinality estimate every 64 elements after the first
	// 		// 1000.
	// 		// const cardinatlityCheckFrequency = 64
	// 		// if i%cardinatlityCheckFrequency == 0 {
	// 		if i%minValsPerDistinctAbbr == 0 {
	// 			card := float64(hll.Estimate())
	// 			// TODO: Document this.
	// 			// If the ratio of distinct values to cardinality less than
	// 			if card < float64(i+1)/minValsPerDistinctAbbr {
	// 				return nil
	// 			}
	// 			// If we've reached 100_000 elements, building the abbreviated
	// 			// values is likely to pay-off. We should continue to build them
	// 			// without the overhead of estimating the cardinality.
	// 			if card > 100_000 {
	// 				break
	// 			}
	// 		}
	...
	for ; i < int(n); i += 8 {
		...
		// if i%minValsPerDistinctAbbr == 0 {
		// 	c := sk.Cardinality()
		// 	if float64(c) < float64(i+1)/minValsPerDistinctAbbr {
		// 		return nil
		// 	}
		// 	if c > 100_000 {
		// 		break
	...
	// Abbreviate the remaining values, without the overhead of HLL cardinality
	// estimation.
	for ; i < int(n); i++ {
		bs := b.Get(i)
		r[i] = abbreviate(bs)
