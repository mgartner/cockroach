...
func (c *CustomFuncs) RejectNullCols(in memo.RelExpr) opt.ColSet {
	return DeriveRejectNullCols(c.mem.Metadata(), in, c.f.disabledRules)
}
...
func DeriveRejectNullCols(
	md *opt.Metadata, in memo.RelExpr, disabledRules intsets.Fast,
) opt.ColSet {
	...
	case opt.InnerJoinOp, opt.InnerJoinApplyOp:
		...
		if in.Child(0).(memo.RelExpr).Relational().OuterCols.Empty() {
			relProps.Rule.RejectNullCols.UnionWith(
				DeriveRejectNullCols(md, in.Child(0).(memo.RelExpr), disabledRules),
			)
		}
		if in.Child(1).(memo.RelExpr).Relational().OuterCols.Empty() {
			relProps.Rule.RejectNullCols.UnionWith(
				DeriveRejectNullCols(md, in.Child(1).(memo.RelExpr), disabledRules),
			)
		}
	...
	case opt.GroupByOp, opt.ScalarGroupByOp:
		...
		relProps.Rule.RejectNullCols.UnionWith(deriveGroupByRejectNullCols(md, in, disabledRules))
	...
	case opt.ProjectOp:
		...
		relProps.Rule.RejectNullCols.UnionWith(deriveProjectRejectNullCols(md, in, disabledRules))
...
func deriveGroupByRejectNullCols(
	...
	for i := range aggs {
		...
		if !DeriveRejectNullCols(md, input, disabledRules).Contains(inColID) {
...
func deriveProjectRejectNullCols(
	...
	rejectNullCols := DeriveRejectNullCols(md, in.Child(0).(memo.RelExpr), disabledRules)
