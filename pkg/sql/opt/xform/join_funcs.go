...
func (c *CustomFuncs) GenerateMergeJoins(
	...
	orders := ordering.DeriveRestrictedInterestingOrderings(
		c.e.mem.Metadata(), left, leftCols,
	).Copy()
	...
	if !c.NoJoinHints(joinPrivate) || c.e.evalCtx.SessionData().ReorderJoinsLimit == 0 {
		...
		rightOrders := ordering.DeriveInterestingOrderings(c.e.mem.Metadata(), right).Copy()
...
func (c *CustomFuncs) LookupsAreLocal(
	...
	_, provided := distribution.BuildLookupJoinLookupTableDistribution(
		...
		lookupJoinExpr,
		required,
		c.e.o.MaybeGetBestCostRelation,
	)
