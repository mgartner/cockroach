...
func (c *CustomFuncs) GenerateInvertedIndexScans(
	...
	iter.ForEach(func(index cat.Index, filters memo.FiltersExpr, indexCols opt.ColSet, _ bool, _ memo.ProjectionsExpr) {
		...
		if invColTypeFamily == types.JsonFamily || invColTypeFamily == types.ArrayFamily {
			if newSpanExpr, ok := reduceInvertedSpans(c.e.ctx, input, scanPrivate.Table, index, spanExpr); ok {
...
// reduceInvertedSpans attempts to
func reduceInvertedSpans(
	...
	if !ok || colStat.Histogram == nil {
		...
		// TODO(mgartner): We could blindly reduce the spans, which will
		// probably be better than doing nothing.
		return nil, false
	}
	histogram := colStat.Histogram

	var lowestCardinality float64
	var findLowestCardinalitySpan func(expr inverted.Expression)
	findLowestCardinalitySpan = func(expr inverted.Expression) {
		span, ok := expr.(*inverted.SpanExpression)
		if !ok {
		...
		case inverted.SetIntersection:
			...
			if len(span.FactoredUnionSpans) > 0 {
				// TODO: Determine if we can handle factored union spans.
				return
			}
			findLowestCardinalitySpan(span.Left)
			findLowestCardinalitySpan(span.Right)
