...
func getCRBDRegionColSetFromInput(
	...
	if bestCostInputRel, ok := maybeGetBestCostRelation(join.Input, required); ok {
		...
		if lookupJoinExpr, ok := maybeScan.(*memo.LookupJoinExpr); ok {
			crdbRegionColSet, inputDistribution =
				BuildLookupJoinLookupTableDistribution(
					ctx, md, evalCtx, lookupJoinExpr, required, maybeGetBestCostRelation)
		...
		inputDistribution =
			BuildProvided(ctx, md, evalCtx, scanExpr, &required.Distribution)
...
func BuildLookupJoinLookupTableDistribution(
	...
	localCrdbRegionColSet, inputDistribution :=
		getCRBDRegionColSetFromInput(ctx, md, evalCtx, lookupJoin, required, maybeGetBestCostRelation)

	lookupTableMeta := md.TableMeta(lookupJoin.Table)
	...
	if lookupJoin.LocalityOptimized || lookupJoin.ChildOfLocalityOptimizedSearch {
		...
		if len(lookupJoin.KeyCols) > 0 {
			...
			if filterIdx, ok := lookupJoin.GetConstPrefixFilter(md); ok {
			...
			} else if lookupJoin.LookupIndexPrefixIsEquatedWithColInColSet(md, localCrdbRegionColSet) {
...
func BuildInvertedJoinLookupTableDistribution(
	ctx context.Context, md *opt.Metadata, evalCtx *eval.Context, invertedJoin *memo.InvertedJoinExpr,
) (provided physical.Distribution) {
	lookupTableMeta := md.TableMeta(invertedJoin.Table)
	lookupTable := lookupTableMeta.Table
